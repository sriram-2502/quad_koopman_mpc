...
cfg.mpc_params["type"] = "koopman_error"
cfg.mpc_params["horizon"] = 15
# Sim timing
cfg.simulation_params["dt"] = 0.002
cfg.simulation_params["mpc_frequency"] = 100   
...
if __name__ == "__main__":
    logdir = REPO_ROOT / "simulation" / "koopman_error_logs"
    logdir.mkdir(parents=True, exist_ok=True)

    out_h5 = run_simulation(
        qpympc_cfg=cfg,
        num_episodes=1,
        num_seconds_per_episode=30.0,
        ...
        recording_filename="koopman_error_experiment.h5",
    )
    ...
    plot_episode_states(
        ...
        title_suffix="Koopman error MPC",
    )

    plot_episode_grfs(
        Path(out_h5), episode_idx=0,
        prefer_cmd=True, fz_threshold=5.0, fz_ylim=None,
        title_suffix="Koopman errorMPC",
    )
